        ...
        for (int b = 0; b < (int)record.summary.nsum; b++){ // Loop over summaries
            ...
            if (pl->num==0 || sum->tar != pl->targets[m].code){
                if (pl->num <= pl->allocated_num){
                    ...
                    pl->targets = (spk_target*) realloc(pl->targets, pl->allocated_num*sizeof(spk_target));
                }
                m++;
...
int assist_spk_calc(spk_s *pl, double jde, double rel, int m, double* GM, double* out_x, double* out_y, double* out_z, double* out_vx, double* out_vy, double* out_vz)
{
    if(pl==NULL){
    ...
    if (jde + rel < target->beg || jde + rel > target->end){
        throw std::runtime_error("The requested time is outside the coverage provided by the ephemeris file.");
    }
...
int assist_jpl_calc(jpl_s *jpl, double jd_ref, double jd_rel, int body, 
    ...
    // if body is less than 0 or not one of 0,1,199,2,299,3,301,399,4,5,6,7,8,9,10, throw error
    if(body<0 || (body!=0 && body!=1 && body!=199 && body!=2 && body!=299 && body!=3 && body!=301 && body!=399 && body!=4 && body!=5 && body!=6 && body!=7 && body!=8 && body!=9 && body!=10)){
        throw std::runtime_error("The requested spice ID has not been found.");
    }
    mpos_s pos;
    ...
    switch (body) { // The indices in the jpl-> arrays match the JPL component index for the body
        ...
        case 0:
            // assist_jpl_work(&z[jpl->off[0]], jpl->ncm[0], jpl->ncf[0], jpl->niv[0], t, jpl->inc, pos.u, pos.v, pos.w);
            *GM = 0.0;
            break;
        // sun
        case 10:
        ...
        case 9:
            ...
            *GM = jpl->mass[10];
            break;
        default:
            throw std::runtime_error("The requested spice ID has not been found."); // body not found
            break;
    }
